# Microsoft Entra ID SAML Single Sign-On Lab

This lab demonstrates how to configure SAML-based Single Sign-On (SSO) using Microsoft Entra ID.

The objective of this project was to create an enterprise application, assign a user to the application, configure SAML authentication settings, and verify the SSO login process.

---

## Tools Used

Microsoft Entra ID (Azure AD)

---

## Lab Steps

1. Created an Enterprise Application using the Microsoft Entra SAML Toolkit.
2. Assigned a user account to the enterprise application.
3. Enabled SAML authentication.
4. Configured the Identifier, Reply URL, and Sign-on URL.
5. Tested the Single Sign-On authentication process.

---

## Enterprise Application Created
![Figure 1](Figure 1. Microsoft Entra enterprise application created for SSO testing.png)

## User Selected for Assignment
![Figure 2](Figure 2. Selecting a User to assign access to the enterprise application.png)

## User Ready for Assignment
![Figure 3](Figure 3. User selected and ready to be assigned to the enterprise application.png)

## User Successfully Assigned
![Figure 4](Figure 4. User successfully assigned to the enterprise application.png)

## SAML Selected as Authentication Method
![Figure 5](Figure 5. Selecting a SAML as the single sign-on method.png)

## Basic SAML Configuration
![Figure 6](Figure 6. Basic SML configuration completed.png)

## Successful SAML SSO Test
![Figure 7](Figure 7. Successful SAML Single sign-on authentication test.png)

## Token Signing Certificate
![Figure 8](Figure 8. SAML token signing certificate configuration.png)

---

## Skills Demonstrated

• Microsoft Entra ID configuration
• Identity and Access Management (IAM)
• SAML authentication configuration
• Enterprise application configuration
• Single Sign-On (SSO)
